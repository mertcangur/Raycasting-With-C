...
/*   Updated: 2024/01/15 17:28:36 by mgur             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map(char *str, t_cub3d **cub3d)
{
	...
	while (check != NULL && !(*cub3d)->map.check)
	{
		if ((is_just_space(check) && check[0] != '\0') \
		|| ((!(check[0] == '\0') && check_map2(check, cub3d) == 1)))
		{
			let_free((*cub3d));
			ft_error("error");
			exit(1);
...
void	check_map_clone(char *str, t_cub3d **cub3d, int line)
{
	...
	if ((*cub3d)->map.direct == 'G' \
	|| check_map_valid(&(*cub3d)->map, &(*cub3d)->player))
	{
		let_free((*cub3d));
